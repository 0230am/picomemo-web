...
await assertPinnedGit(picomemo, lock.picomemo.commit, lock.picomemo.tree, "picomemo source");
...
const tests = gitShow("test/omemo.c").replace('#include "../omemo.c"', `#include "${omemoPath}"`);
...
function compile(source, output, includeCore) {
	const args = [
		source,
